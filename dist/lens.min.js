...
r=e=>{VYLO.global.Lens=e;window.Lens=e;e.version="v1.0.0";e.cachedResourcesInfo={};e.init=!0;e.attached=!1;e.isMoving=!1;e.isZooming=!1;e.isSpectating=!1;e.isShaking=!1;e.isScrolling=!1;e.isPanning=!1;Diob.prototype.getTrueCenterPos||(Diob.prototype.getTrueCenterPos=function(){const f=VYLO.World.getTileSize();return{x:Math.round(this.x+(this.aIconInfo?this.aIconInfo.halfWidth:f.width)+this.xIconOffset),y:Math.round(this.y+(this.aIconInfo?this.aIconInfo.halfHeight:f.height)+this.yIconOffset)}});EListener.on(VYLO.Client,
"onScreenRender",function(f){if(e.init){const a=Date.now();this.___EVITCA_aPause&&aPause&&aPause.paused||(e.settings.loop.lastTime||(e.settings.loop.lastTime=a),f=(a-e.settings.loop.lastTime)/1E3,e.settings.loop.elapsedMS=a-e.settings.loop.lastTime,.033<f&&(f=.033),e.settings.loop.deltaTime=f,this.___EVITCA_aParallax&&e.attached&&this.aParallax.update(e.following.getTrueCenterPos().x-e.oldPos.x,e.following.getTrueCenterPos().y-e.oldPos.y),e.update(e.settings.loop.elapsedMS,e.settings.loop.deltaTime));
e.settings.loop.lastTime=a}});VYLO.Client.attachCamera=function(f){e.settings.zoom.currentLevel.x=this.mapView.scale.x;e.settings.zoom.currentLevel.y=this.mapView.scale.y;e.assignIconSize(this.mob);e.following=this.mob;e.setPos(this.mob.getTrueCenterPos().x,this.mob.getTrueCenterPos().y,this.mob.mapName);e.oldPos.x=e.xPos;e.oldPos.y=e.yPos;e.attached=!0;f?"object"===typeof f&&f.constructor===Object?(f.duration?"object"===typeof f.duration&&f.duration.constructor===Object?"number"===typeof f.duration.x&&
"number"===typeof f.duration.y?(e.settings.custom.duration.x=f.duration.x,e.settings.custom.duration.y=f.duration.y):(e.settings.custom.duration.x=e.settings.custom.duration.y=1E3,this.debugging&&console.warn("Lens: Invalid variable type passed for the %cpSettings.duration.x || pSettings.duration.y","font-weight: bold","property. Reverted to default")):typeof("number"===f.duration)?e.settings.custom.duration.x=e.settings.custom.duration.y=f.duration:(e.settings.custom.duration.x=e.settings.custom.duration.y=
1E3,this.debugging&&console.warn("Lens: Invalid variable type passed for the %cpSettings.duration","font-weight: bold","property. Reverted to default")):(e.settings.custom.duration.x=e.settings.custom.duration.y=1E3,this.debugging&&console.warn("Lens: No %cpSettings.duration","font-weight: bold","parameter passed. Reverted to default")),f.ease?"object"===typeof f.ease&&f.ease.constructor===Object?"string"===typeof f.ease.x&&"string"===typeof f.ease.y?k.includes(f.ease.x)&&k.includes(f.ease.y)?(e.settings.custom.ease.x=
f.ease.x,e.settings.custom.ease.y=f.ease.y):(e.settings.custom.ease.x=e.settings.custom.ease.y="easeOutCubic",this.debugging&&console.warn("Lens: Invalid %cease","font-weight: bold","name passed for pSettings.ease.x || pSettings.ease.y. Reverted to default")):(e.settings.custom.ease.x=e.settings.custom.ease.y="easeOutCubic",this.debugging&&console.warn("Lens: Invalid variable type passed for the %cpSettings.ease.x || pSettings.ease.y","font-weight: bold","property. Reverted to default")):typeof("string"===
f.ease)?k.includes(f.ease)?e.settings.custom.ease.x=e.settings.custom.ease.y=f.ease:(e.settings.custom.ease.x=e.settings.custom.ease.y="easeOutCubic",this.debugging&&console.warn("Lens: Invalid %cease","font-weight: bold","name passed for pSettings.ease. Reverted to default")):(e.settings.custom.ease.x=e.settings.custom.ease.y="easeOutCubic",this.debugging&&console.warn("Lens: Invalid variable type passed for the %cpSettings.ease","font-weight: bold","property. Reverted to default")):(e.settings.custom.ease.x=
e.settings.custom.ease.y="easeOutCubic",this.debugging&&console.warn("Lens: No %cpSettings.ease","font-weight: bold","parameter passed. Reverted to default")),e.reset("standard"),e.custom=!0,this.following=this.mob,e.settings.custom.initialPos.x=e.xPos,e.settings.custom.initialPos.y=e.yPos):(e.reset("custom"),e.custom=!1,e.following=this.mob,e.settings.standard.initialPos.x=e.xPos,e.settings.standard.initialPos.y=e.yPos,this.debugging&&console.warn("Lens: Invalid variable type passed for the %cpSettings",
